...
1. YOLOv8n trained from scratch
2. YOLOv8n trained using transfer learning
...
> The uploaded notebook was saved without executed outputs, so final numerical validation results are not included here. Run the notebook to generate the actual metrics.
...
These weights already contain useful general visual features learned from a large dataset.

The model then fine-tunes these features for road sign detection.
...
1. Locate images and XML annotation files.
2. Extract class names.
3. Match images with annotations.
4. Split the data into training and validation sets.
5. Convert Pascal VOC annotations into YOLO format.
...
The project demonstrates the practical difference between training a deep-learning object detector from random initialization and adapting a pretrained model through transfer learning.
